...
#src-git luci $github_mirror/coolsnowwolf/luci.git;openwrt-23.05   #23.05分支luci
src-git luci $github_mirror/coolsnowwolf/luci$luci      #master分为luci18
...
EOF
	...
	# 重命名固件 格式：OprX-lede-x86_64-eS-R24.9.18-2024091815-Fsquashfs-uefi-c347f.img.gz
	...
	#提取lede的现有版本号
	#curl -s https://raw.githubusercontent.com/coolsnowwolf/lede/master/package/lean/default-settings/files/zzz-default-settings > ota/zzz-default-settings
	#Version_File=ota/zzz-default-settings
    ##zzz_Default_Version="$(egrep -o "R[0-9]+\.[0-9]+\.[0-9]+" ${Version_File})"  #提取出来是带R的
    #zzz_Default_Version="$(egrep -o "R[0-9]+\.[0-9]+\.[0-9]+" ${Version_File} | sed 's/R//g')"   #提取出来是不带R的
	#OP_VERSION="R${zzz_Default_Version}-${Build_DATE}"   #这里根据上面情况看不带R
	...
	rm -rf ota/zzz-default-settings 
